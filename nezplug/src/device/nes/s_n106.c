...
static void __fastcall N106SoundWriteAddr(void *pNezPlay, Uint address, Uint value)
{
	...
	Uint8 data = value;
	vgm_write_large_data(n106s->vgm_idx, 0x01, 0x10000, address, 1, &data);
}
...
*/
	...
	if (n106s->addressauto)
	{
		if (n106s->dataread)
		{
			/* オートインクリメント中にデータが読まれるとアドレスがズレるので再設定する */
			Uint8 data = 0x80 | n106s->address;
			vgm_write_large_data(n106s->vgm_idx, 0x01, 0x10000, N106SOUND_ADDR, 1, &data);
			n106s->dataread = 0;
		}
